...
The first full pilot assembled — sixty seconds, six shots, every physics gate green, delta 0.301s — and the creator's note read: no storytelling in the shot. The measurements agreed once we knew how to read them: all five joins landed at 0-3 of 64 bits because every boundary went to the model as a first-frame continuation; the declared setups arrived by drift instead of by cut, and the film was one unbroken take wearing six shots' names. The lesson entered the law the signed way: plans now declare every join as a cut or continuous (the breakdown refuses an untyped join), the executor chains frames only across continuous joins and sends bible plates on cuts, CIN-003 now governs only continuous joins, CIN-012 was born from the note (a cut must change the image — calibrating, with the pilot's 0-3 distances as its no-change population), SCR-012 requires every shot's moment to name a change, and the first learned regression case landed in the director suite. The metrics measured physics; the note taught them to start measuring drama.

The next chapter is planned, not started: docs/PIPELINE.md is the design for Pipeline V2 — a command-line, no-human-in-the-run pipeline (idea + style + seconds in; film slice + errors database out) with a policy rulebook replacing approval, a rubric-bound witness judge whose authority is earned by calibration against the creator's notes, a chain-parallel scheduler with reservations before every paid call, a required ledger with intent/result pairs so a killed run never pays twice, and five named lanes from the errors database to harness change. Five milestones with exit criteria; it waits on the creator's go plus one style file and one policy file.

Pipeline V2 is built and green on the stubbed wire (18 suites) and has run once for real: the first autonomous pass planned 25 shots with subject, force and change, approved itself under an unbounded policy, rendered plates, and halted because the creator's dev server predated two new routes and answered them with HTML — every plate was re-paid five times against that one 404 before the completion refused a plate node. Both defects are fixed (a rendered plate now survives a QC fault; an exhausted plate ships unjudged or is marked absent; a non-JSON route answer is a named fault), unverified until the creator runs the suites. The creator then simplified the engine: the default `bravo film` path is now the edit loop in `agents/director/editloop.js` — every shot renders as one 20-30 second take in parallel, is measured and QC'd, and a failing take is judged (keep, edit, regenerate, with a reason) and edited by Seedance 2.5 on the source clip or regenerated within five attempts; the best ships with a named shortfall; stitch, timeline gate, film QC and report follow. Plates, keyframe candidates and the chain-parallel schedule remain as `--mode staged`. The asset-library stage is specified in PIPELINE.md and not built. Nine reviewed ledger-precision findings remain open and documented in the plan's review notes.
